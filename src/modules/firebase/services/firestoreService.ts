...
export class FirestoreService<T extends any> {
  ...
  /** Add new documents with passed id */
  // async addAsync(entity: T | T[]) {
  async setAsync(entity: T) {
  ...
  async getSubCollectionAsync(documentId: string, subcollection: string) {
    return await this.collection
      .doc(documentId)
      .collection(subcollection)
      .get()
